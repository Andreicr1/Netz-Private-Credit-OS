...
class CashTransaction(Base, IdMixin, FundScopedMixin, AuditMetaMixin):
    ...
    policy_basis: Mapped[list[dict] | None] = mapped_column(JSON, nullable=True)  # list[{document_id, section, excerpt}]

    investment_memo_document_id: Mapped[uuid.UUID | None] = mapped_column(ForeignKey("documents.id", ondelete="SET NULL"), nullable=True)
    ic_approvals_count: Mapped[int] = mapped_column(Integer, nullable=False, server_default="0")
    ic_approval_evidence: Mapped[dict | None] = mapped_column(JSON, nullable=True)

    # Admin/bank execution tracking
    sent_to_admin_at: Mapped[datetime | None] = mapped_column(DateTime(timezone=True), nullable=True)
    ...
    # Evidence pointers
    instructions_blob_uri: Mapped[str | None] = mapped_column(String(800), nullable=True)
    evidence_bundle_blob_uri: Mapped[str | None] = mapped_column(String(800), nullable=True)
    evidence_bundle_sha256: Mapped[str | None] = mapped_column(String(64), nullable=True)
...
class CashTransactionApproval(Base, IdMixin, FundScopedMixin, AuditMetaMixin):
    ...
    transaction_id: Mapped[uuid.UUID] = mapped_column(ForeignKey("cash_transactions.id", ondelete="CASCADE"), index=True)
    approver_role: Mapped[str] = mapped_column(String(32), nullable=False)  # DIRECTOR / IC_MEMBER
    ...
    __table_args__ = (Index("ix_cash_approvals_tx_role", "transaction_id", "approver_role"),)
