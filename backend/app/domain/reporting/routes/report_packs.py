...
from app.domain.reporting.enums import ReportPackStatus, ReportSectionType
...
from app.domain.reporting.schemas.report_packs import ReportPackCreate, ReportPackOut
from app.domain.reporting.services.generators import (
...
@router.post("/funds/{fund_id}/report-packs", response_model=ReportPackOut)
def create_pack(
    fund_id: uuid.UUID,
...
def generate_pack(
    ...
    pack = db.query(MonthlyReportPack).filter(MonthlyReportPack.fund_id == fund_id, MonthlyReportPack.id == pack_id).first()
    if not pack:
