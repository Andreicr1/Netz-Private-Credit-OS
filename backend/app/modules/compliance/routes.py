...
def snapshot(
    ...
    _role_guard: Actor = Depends(require_roles([Role.COMPLIANCE, Role.ADMIN, Role.AUDITOR, Role.INVESTMENT_TEAM])),
) -> ComplianceSnapshotOut:
    obligations = list(db.execute(select(Obligation.id).where(Obligation.fund_id == fund_id)).scalars().all())
    wf = service.get_workflow_status_map(db, fund_id=fund_id, obligation_ids=obligations)
    ...
    total_gaps = int(
        db.execute(
            select(Obligation.id).where(Obligation.fund_id == fund_id, Obligation.name.like(f"{AI_GAP_PREFIX}%"))
        ).scalars().all().__len__()
    )
...
def list_obligations(
    ...
    items = service.list_obligations(db, fund_id=fund_id, limit=limit, offset=offset)
    wf = service.get_workflow_status_map(db, fund_id=fund_id, obligation_ids=[o.id for o in items])

    out: list[ObligationWorkflowOut] = []
    for o in items:
        ow = ObligationWorkflowOut.model_validate(o)
        ow.workflow_status = wf.get(o.id, "OPEN")
        if view == "active" and ow.workflow_status == "CLOSED":
            continue
        if view == "closed" and ow.workflow_status != "CLOSED":
            continue
        out.append(ow)

    return Page(items=out, limit=limit, offset=offset)
...
def get_obligation(
    ...
    try:
        ...
        wf = service.get_workflow_status_map(db, fund_id=fund_id, obligation_ids=[ob.id])
...
@router.get("/obligations/{obligation_id}/evidence", response_model=list[ObligationEvidenceOut])
def list_evidence(
    fund_id: uuid.UUID,
    obligation_id: uuid.UUID,
    db: Session = Depends(get_db),
    ...
    try:
        events = service.list_linked_evidence(db, fund_id=fund_id, obligation_id=obligation_id)
    except ValueError as e:
        raise HTTPException(status_code=404, detail=str(e))
    ...
    for ev in events:
        ...
        out.append(
            ObligationEvidenceOut(
                ...
                version_id=uuid.UUID(after["version_id"]) if after.get("version_id") else None,
...
@router.post("/obligations/{obligation_id}/evidence/link", status_code=status.HTTP_201_CREATED)
def link_evidence(
    fund_id: uuid.UUID,
    obligation_id: uuid.UUID,
...
def mark_in_progress(
    ...
    try:
        service.mark_in_progress(db, fund_id=fund_id, actor=actor, obligation_id=obligation_id)
...
def close_obligation(
    ...
    try:
        service.close_obligation(db, fund_id=fund_id, actor=actor, obligation_id=obligation_id)
...
def obligation_audit(
    ...
    try:
        return service.get_obligation_audit(db, fund_id=fund_id, obligation_id=obligation_id)
    except ValueError as e:
        raise HTTPException(status_code=404, detail=str(e))
