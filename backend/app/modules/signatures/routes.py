...
def list_signature_requests(
    ...
    _role_guard: Actor = Depends(
        require_roles([Role.DIRECTOR, Role.GP, Role.COMPLIANCE, Role.INVESTMENT_TEAM, Role.AUDITOR, Role.ADMIN])
    ),
...
def get_signature_request(
    ...
    tx = db.execute(select(CashTransaction).where(CashTransaction.fund_id == fund_id, CashTransaction.id == request_id)).scalar_one_or_none()
    if not tx:
...
def sign_request(
    ...
    out = service.sign(db, fund_id=fund_id, actor=actor, tx_id=request_id, comment=payload.comment)
    return SignatureRequestDetailOut(**out)
...
def reject_request(
    ...
    out = service.reject(db, fund_id=fund_id, actor=actor, tx_id=request_id, reason=payload.reason)
    return SignatureRequestDetailOut(**out)
...
def execution_pack(
    ...
    actor: Actor = Depends(get_actor),
    _write_guard: Actor = Depends(require_readonly_allowed()),
    _role_guard: Actor = Depends(require_roles([Role.DIRECTOR, Role.GP, Role.COMPLIANCE, Role.ADMIN])),
) -> dict:
    return service.generate_execution_pack(db, fund_id=fund_id, actor=actor, tx_id=request_id)
